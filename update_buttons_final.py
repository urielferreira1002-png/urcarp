...
def update_buttons(file_path):
    ...
    # Primeiro, garantir que o botão de compra seja um link para Discord
    # Substituir qualquer button com onclick comprar* por link
    buy_pattern = r'<button class="btn btn-comprar" onclick="comprar[^"]*">[^<]*</button>'
    buy_replacement = '<a href="https://discord.gg/ZgWVdm4H" target="_blank" class="btn btn-comprar">🛒 COMPRAR AGORA</a>'

    content = re.sub(buy_pattern, buy_replacement, content)

    # Remover o botão/link separado do Discord
    # Padrão: <a href="https://discord.gg/ZgWVdm4H" target="_blank" class="btn btn-discord">💬 DISCORD</a>
    discord_pattern = r'\s*<a href="https://discord\.gg/ZgWVdm4H" target="_blank" class="btn btn-discord">💬 DISCORD</a>\s*'
    content = re.sub(discord_pattern, '', content)
